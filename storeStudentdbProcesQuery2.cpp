/*First I stored the student db in a binary file and then I wrote query engine for that.. Queries should be in an order as follows..
-> There is no need to give table name(since there is only one table i have written)
 eg: select <attribute> where <condition>;
-> only one condition can be executed
-> You can ask for different attributed to print but if u want all attribute values just give '*'
->attribute values should be given only with structure identifiers
-> condition values should be given in quotes even if it is an integer
 eg: select no,user where n0="2345";
-> statement should be ended with ';' 
-> try giving only valid inputs */
